...
bool ChatHandler::HandleStartCommand(const char* args, WorldSession *m_session)
{
	...
	if(info == NULL)
	{
		RedSystemMessage(m_session, "Error! Couldn't find [playercreateinfo]");
		return false;
	}


	GreenSystemMessage(m_session, "Transporting player [%s] to his [%s]'s starting location...", m_plyr->GetName(), race.c_str());
...
bool ChatHandler::HandleInfoCommand(const char* args, WorldSession* m_session)
{
	...
	SystemMessage(m_session, "Latency |r%.3fms", ((float)avg / (float)count));
	SystemMessage(m_session, "Developed by Zerocool, WoWD, Milamba and Hendrik");
	SystemMessage(m_session, "Uptime |r%s", sWorld.GetUptimeString().c_str());
	SystemMessage(m_session, "Players Online |r%dplayers %d Gamemasters", count, gm);
	SystemMessage(m_session, "Active Thread Count |r%u", ThreadPool.GetActiveThreadCount());
	SystemMessage(m_session, "CPU Usage %3.2f %%", sWorld.GetCPUUsage());
	SystemMessage(m_session, "WOWD Query Cache Size |r%u queries delayed", WorldDatabase.GetQueueSize());
	SystemMessage(m_session, "CHAR Query Cache Size: |r%u queries delayed", CharacterDatabase.GetQueueSize());
